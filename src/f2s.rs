...
pub unsafe fn f2s_buffered_n(f: f32, result: *mut u8) -> usize {
    ...
    let ieee_exponent = ((bits >> FLOAT_MANTISSA_BITS) & ((1u32 << FLOAT_EXPONENT_BITS) - 1)) as u32;
        ...
        || (ieee_exponent == 0 && ieee_mantissa == 0)
    {
        ...
        e2 = 1 - offset as i32 - FLOAT_MANTISSA_BITS as i32 - 2;
        m2 = ieee_mantissa;
    } else {
        e2 = ieee_exponent as i32 - offset as i32 - FLOAT_MANTISSA_BITS as i32 - 2;
    ...
    let mv = 4 * m2;
    let mp = 4 * m2 + 2;
    let mm = 4 * m2 - if m2 != (1u32 << FLOAT_MANTISSA_BITS) || ieee_exponent <= 1 { 2 } else { 1 };
    ...
    if e2 >= 0 {
        ...
        if q != 0 && (vp - 1) / 10 <= vm / 10 {
            ...
            last_removed_digit = (mul_pow5_inv_div_pow2(mv, (q - 1) as u32, -e2 + q - 1 + l) % 10) as u8;
        }
        if q <= 9 {
    ...
    if vm_is_trailing_zeros || vr_is_trailing_zeros {
        ...
        output = vr +
            ((vr == vm && (!accept_bounds || !vm_is_trailing_zeros)) || (last_removed_digit >= 5)) as u32;
    } else {
        // Common case.
        while vp / 10 > vm / 10 {
            last_removed_digit = (vr % 10) as u8;
